	...
	ajax(
		  {
	    		...
	    		type:"json",
	    		callback:function(data) {
					// 循环遍历每个用户分类，每个名称生成一个option对象，添加到<select>中
					for(var index in data) {
						var op = document.createElement("option");//创建一个指名名称元素
						op.value = data[index].readerTypeId;//设置op的实际值为当前的用户分类编号
...
function validUpdateReader() {
    ...
    if (paperNO == "") {
        ...
        $('#updatePaperNO').next().text("请输入用户证件号");
	...
	if(readerType == -1){
        ...
        $('#updateReaderType').next().text("请选择用户类型");
