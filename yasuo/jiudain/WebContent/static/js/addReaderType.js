...
function validAddReaderType() {
	...
	if(typeName == ""){
        ...
        $('#addType').next().text("请输入用户类型名称");
        $("#addType").next().show();
        flag = false;
	}else if(!reg.test(typeName)){
		 $('#addType').parent().addClass("has-error");
        $('#addType').next().text("用户类型名称必须为中文");
        $("#addType").next().show();
		 flag = false;
	}else {
	...
	if(maxNum == ""){
        ...
        $('#addMaxNum').next().text("请输入最大预订数量");
        $("#addMaxNum").next().show();
        flag = false;
	}else if(maxNum<=0 || maxNum!=parseInt(maxNum)){
    	$('#addMaxNum').parent().addClass("has-error");
        $('#addMaxNum').next().text("最大预订数量必须为正整数");
        $("#addMaxNum").next().show();
        flag = false;
	} else {
	...
	if(bday == ""){
        ...
        $('#addBday').next().text("请输入最大预订天数");
        $("#addBday").next().show();
        flag = false;
	}else if(bday<=0 || bday!=parseInt(bday)){
    	$('#addBday').parent().addClass("has-error");
        $('#addBday').next().text("最大预订天数必须为正整数");
        $("#addBday").next().show();
        flag = false;
	} else {
