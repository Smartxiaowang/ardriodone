...
<%@ page import = "bean.TypeBean,dao.AdminDao,dao.TypeDao,dao.JiudianDao,bean.AdminBean" %>
                    ...
                    <div class="col-lg-12">
                        <div class="panel panel-default bootstrap-admin-no-table-panel">
                            <div class="panel-heading">
                                <div class="text-muted bootstrap-admin-box-title">客房分类管理</div>
                            </div>
                            <div class="bootstrap-admin-no-table-panel-content bootstrap-admin-panel-content collapse in">
                                <form class="form-horizontal" action="/jiudain/AddJiudianTypeServlet" method="post">
                        
                                    <div class="col-lg-3 form-group">

                                        <button type="button" class="btn btn-primary" id="btn_add" data-toggle="modal" data-target="#addJiudianType">添加分类</button>
                                    </div>
                                </form>
                            </div>
                        </div>
...
//                           if(typedata==null){
                             TypeDao jiudiandao = new TypeDao();
                             typedata = (ArrayList<TypeBean>)jiudiandao.get_ListInfo();
//                           }
...
<button type="button" class="btn btn-danger btn-xs" onclick="deletejiudianType(<%= bean.getTid() %>)">删除</button>
	</td>                                            
                          	  </tbody>
                       <%} %> 
                        </table>
    ...
    function deletejiudianType(tid) {
    	con=confirm("是否删除?"); 
    	if(con==true){
    		location.href = "/jiudain/deleteTypeServlet?tid="+tid;
    	}
    }
    </script>
                                     ...
                                     <!-------------------------------------------------------------->  
                                        ...
                                        <!-- 修改模态框（Modal） -->
                               <form class="form-horizontal" method="post" action="/jiudain/updateJiudianTypeServlet">   <!--保证样式水平不混乱-->
										 ...
										 <div class="form-group">
											<label for="firstname" class="col-sm-3 control-label">客房名称</label>
												...
												<input type="hidden" name="tid" id="updatetid">
													<input type="text" class="form-control" id="updatename" name="name"  placeholder="请输入客房分类名称">
     ...
     <!--------------------------------------添加的模糊框------------------------>  
                                 <form class="form-horizontal" method="post" action="/jiudain/AddJiudianTypeServlet">   <!--保证样式水平不混乱-->
                                        <!-- 模态框（Modal） -->
									<div class="modal fade" id="addJiudianType" tabindex="-1" role="dialog" aria-labelledby="addJiudianTypeLabel" aria-hidden="true">
										<div class="modal-dialog">
											<div class="modal-content">
												<div class="modal-header">
													<button type="button" class="close" data-dismiss="modal" aria-hidden="true">
													...
													<h4 class="modal-title" id="addJiudianTypeLabel">
														添加新客房分类
													</h4>
												</div>
												<div class="modal-body">
										...
										<!---------------------表单-------------------->
										 ...
										 <div class="form-group">
											<label for="firstname" class="col-sm-3 control-label">分类名称</label>
												<div class="col-sm-7">
													<input type="text" class="form-control" id="addJiudianName" required="required" name="name"  placeholder="请输入客房分类名称">
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-body">
								...
								<input type="hidden" name="url" value="admin_jiudiantype">
							<div class="form-group">
