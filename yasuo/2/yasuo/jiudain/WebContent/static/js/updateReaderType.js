...
function validUpdateReaderType() {
	...
	if(typeName == ""){
        ...
        $('#readerTypeName').next().text("请输入用户类型名称");
        $("#readerTypeName").next().show();
        flag = false;
	}else if(!reg.test(typeName)){
		 $('#readerTypeName').parent().addClass("has-error");
        $('#readerTypeName').next().text("用户类型名称必须为中文");
        $("#readerTypeName").next().show();
		 flag = false;
	}else {
	...
	if(maxNum == ""){
        ...
        $('#maxNum').next().text("请输入最大预订数量");
        $("#maxNum").next().show();
        flag = false;
	}else if(maxNum<=0 || maxNum!=parseInt(maxNum)){
    	$('#maxNum').parent().addClass("has-error");
        $('#maxNum').next().text("最大预订数量必须为正整数");
        $("#maxNum").next().show();
        flag = false;
	} else {
	...
	if(bday == ""){
        ...
        $('#bday').next().text("请输入最大预订天数");
        $("#bday").next().show();
        flag = false;
	}else if(bday<=0 || bday!=parseInt(bday)){
    	$('#bday').parent().addClass("has-error");
        $('#bday').next().text("最大预订天数必须为正整数");
        $("#bday").next().show();
        flag = false;
	} else {
	...
	if(renewDays == ""){
        ...
        $('#renewDays').next().text("请输入续订天数");
        $("#renewDays").next().show();
        flag = false;
	}else if(renewDays<=0 || renewDays!=parseInt(renewDays)){
    	$('#renewDays').parent().addClass("has-error");
        $('#renewDays').next().text("续订天数必须为正整数");
        $("#renewDays").next().show();
        flag = false;
	} else {
